...
for dTyp=1:3                % interpol, extrapol1, extrapol2
    ...
    for typ=1:2             % 1=without LF, 2=with spatial structure as LF
        ...
        X_t=importdata(file);
        X_t=[ones(size(X_t,1),1), X_t, x2fx(X_t, 'interaction'), X_t.^2];
        %valid
        file=fullfile(folderData,strcat('Xv_', num2str(dTyp), '_', num2str(set_no),'.csv'));
        X_v=importdata(file);
        X_v=[ones(size(X_v,1),1), X_v, x2fx(X_v, 'interaction'), X_v.^2];
        ...
        for sp=1:nsp
            ...
            filebody=strcat('sp',num2str(sp),'_',num2str(set_no),'_hmscIntxn',num2str(typ),'_d',num2str(dTyp),'_',num2str(dSz));
            filebodyPred=strcat('sp',num2str(sp),'_',num2str(set_no),'_pred_hmscIntxn',num2str(typ),'_d',num2str(dTyp),'_',num2str(dSz));
            filebodyPredCsv=strcat('preds_ss_',num2str(set_no),'_hmscIntxn',num2str(typ),'_d',num2str(dTyp),'_',num2str(dSz));
            filebodyTime=strcat('compTime_',num2str(set_no),'_hmscIntxn',num2str(typ),'_',num2str(dSz));
            if MCMC2
