...
for run = startRunN+(1:nRuns)
	...
	for mcmc = 1:m.samVec(end)
		...
		z = Hmsc.update_z(z, X,Xr,Xs,beta,eta,lambda,etas,lambdas,Y,pi,ncr,dist,sigma,speciesX,includeXs,factorCov);
		sigma = Hmsc.update_sigma(X,Xr,Xs,sigma,beta,eta,lambda,etas,lambdas,pi,ncr,dist,asigma,bsigma,z,spatial,factorCov,speciesX,includeXs);
		beta = Hmsc.update_beta(X,Xr,Xs,z,pi,ncr,eta,lambda,etas,lambdas,gamma,sigma,T,ph,iV,rho,phylogeny,iQg,detQg,outlierSpecies,spatial,factorCov,speciesX,includeXs);
		...
		if m.nr > 0
			if (run<m.adapt(1) || (run==m.adapt(1) && mcmc<=m.adapt(2)) ) && m.fixNf==false
