...
for (j in 1:3) {
	...
	for (m in 1:2) {
		...
		load(file=file.path(FD,set_no,paste("gams",modls[m],"_intxn_",j,"_",dataN[sz],".RData",sep="")))
		...
		save(gam_PAs, file = file.path(PD2, set_no,
			paste("gam",modls[m],"_intxn_PAs_",j,"_",dataN[sz],".RData",sep="")))
		rm(gam_PAs)
		gc()
