...
#	(with parameter uncertainty and variable interactions)
##########################################################################################
...
for (j in 1:3) {
	...
	glm1_PAs <- foreach(i=1:nsp)  %dopar% { 
		tryCatch({ predict.glm.sdf(glms1[[i]], predX=DD_v[[j]][[i]], paramUncert=FALSE)},
			error=function(e){cat("ERROR :",conditionMessage(e), "\n")})}

	glm1_PAs <- aperm(simplify2array(glm1_PAs), c(1,3,2))

	save(glm1_PAs, file=paste(PD2,set_no,"/glm1_intxn__PAs_",j,"_",dataN[sz],".RData",sep=""))
	...
	save(glm1b_PAs, file=file.path(PD2,set_no,paste("glm1b_intxn_PAs_",j,"_",dataN[sz],".RData",sep="")))
